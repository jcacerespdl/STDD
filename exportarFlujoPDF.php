...
// Logo + Encabezado
$logo = 'logo-inr-blanco.png'; // Ruta a tu logo
$pdf->Image($logo, 15, 6, 20); // más pequeño y más arriba

$pdf->SetY(16); // ajustar la posición para que quede debajo del logo
...
if ($mov['iCodTramiteDerivar']) {
    // Si es derivación, mostramos el derivado
    ...
    $d = sqlsrv_fetch_array($stmtDoc, SQLSRV_FETCH_ASSOC);
    $docTexto = ($d) ? $d['cDescTipoDoc'] . ' N° ' . $d['cCodificacion'] : '—';
} else {
    // Si es movimiento del documento original
    $docTexto = $data['cDescTipoDoc'] . ' N° ' . $data['cCodificacion'];
}
    
      // Obtener asunto desde el trámite (generado o derivado)
if ($mov['iCodTramiteDerivar']) {
    // Derivado: usar asunto del trámite derivado
    $stmtAsunto = sqlsrv_query($cnx, "SELECT cAsunto FROM Tra_M_Tramite WHERE iCodTramite = ?", [$mov['iCodTramiteDerivar']]);
    $rowAsunto = sqlsrv_fetch_array($stmtAsunto, SQLSRV_FETCH_ASSOC);
    $asunto = $rowAsunto['cAsunto'] ?? '—';
} else {
    // Generado: usar asunto del trámite principal
    $asunto = $data['cAsunto'] ?? '—';
}

// Agregar fase si existe

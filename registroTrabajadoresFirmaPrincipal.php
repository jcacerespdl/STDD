...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST['accion'] === 'eliminar') {
    $iCodTramite = intval($_POST['iCodTramite'] ?? 0);
    $iCodTrabajador = intval($_POST['iCodTrabajador'] ?? 0);

    $sqlDel = "DELETE FROM Tra_M_Tramite_Firma 
               WHERE iCodTramite = ? AND iCodTrabajador = ? AND iCodDigital IS NULL";
    sqlsrv_query($cnx, $sqlDel, [$iCodTramite, $iCodTrabajador]);

    echo "<script>
    alert('Firmante eliminado correctamente.');
    if (window.opener && typeof window.opener.actualizarBotonFirmarPrincipal === 'function') {
        window.opener.actualizarBotonFirmarPrincipal();
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['firmantesJson'])) {
    ...
    foreach ($firmantesData as $firmante) {
        ...
        $tipoFirma = 2; // Solo visto bueno permitido
        $iCodOficina = intval($firmante['iCodOficina']);

        $sqlCheck = "SELECT COUNT(*) AS total FROM Tra_M_Tramite_Firma 
                     WHERE iCodTramite = ? AND iCodDigital IS NULL AND iCodTrabajador = ?";
        $paramsCheck = [$iCodTramite, $idTrab];
        ...
        if ($rowCheck['total'] == 0) {
            ...
            $params = [$iCodTramite, $idTrab, $iCodOficina, $posicion, $tipoFirma];
            sqlsrv_query($cnx, $sqlAdd, $params);
        }
    }

    echo "<script>
    alert('Firmantes insertados correctamente.');
    if (window.opener && typeof window.opener.actualizarBotonFirmarPrincipal === 'function') {
        window.opener.actualizarBotonFirmarPrincipal();
    }
    window.close();
...
<head>
    <meta charset="UTF-8">
    <title>Visto Bueno Documento Principal</title>
...
$sqlFirmantes = "SELECT F.iCodTrabajador, T.cNombresTrabajador, T.cApellidosTrabajador, 
                 ...
                 WHERE F.iCodTramite = ? AND F.iCodDigital IS NULL AND F.tipoFirma = 2
                 ORDER BY F.posicion ASC";
$stmtFirmantes = sqlsrv_query($cnx, $sqlFirmantes, [$iCodTramite]);

$firmantesExistentes = 0;
$firmantesHTML = "";
...
while ($row = sqlsrv_fetch_array($stmtFirmantes, SQLSRV_FETCH_ASSOC)) {
    ...
    $firmantesHTML .= "<tr>
        ...
        <td>Visto Bueno</td>
        <td>
            <form method='POST' onsubmit=\"return confirm('¿Eliminar este firmante?')\">
                ...
                <button type='submit' style='color:red; border:none; background:none; cursor:pointer;' title='Eliminar firmante'>
                    ❌
                </button>
            </form>
        </td>
...
<form method="POST">
    ...
    <div class="input-container select-flotante" style="width: 100%;">
    <input type="text" id="nombreOficina" autocomplete="off"
           placeholder=" "  
           onclick="buscarOficina('')"
           oninput="buscarOficina(this.value)"
           required>
...
<thead><tr><th>Posición</th><th>Nombre</th><th>Oficina</th><th>Tipo Firma</th><th>Acción</th></tr></thead>
    <tbody><?= $firmantesHTML ?></tbody>
</table>

<script>
let seleccionados = [];
let firmantesExistentes = <?= $firmantesExistentes ?>;

function asignarFirma(id, nombre, apellidos, oficinaId, oficinaNombre, perfil) {
    ...
    const posicion = letras[firmantesExistentes + seleccionados.length] || 'Z';

    seleccionados.push({ id, nombre, apellidos, iCodOficina: oficinaId, oficinaNombre, perfil, posicion });
...
function cargarTrabajadores() {
    ...
    fetch(`./ajax_trabajadores_por_oficina2.php?iCodOficina=${oficina}`)
        .then(res => res.json())
        .then(data => {
            ...
            data.forEach(trab => {
                tbody.innerHTML += `<tr id="fila-${trab.id}">
                    ...
                    <td>${trab.perfil}</td>
                    <td>
                        <button class="botonFirma" onclick="asignarFirma('${trab.id}', '${trab.nombre}', '${trab.apellidos}', '${trab.iCodOficina}', '${trab.oficinaNombre}', '${trab.perfil}')">

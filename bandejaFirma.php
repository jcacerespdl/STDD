...
function firmarSeleccionadosLoteComples() {
    ...
    if (data.success) {
        ...
        const param_url = `https://tramite.heves.gob.pe/SGD/getFpParamsMasivoRevision.php?iCodTramite=${nombreZip}`;
        const paramPrev = {
            param_url: param_url,
            param_token: "123456",
...
document.querySelectorAll(".firmarVBPrincipal").forEach(btn => {
    ...
    if (data.success) {
        const nombreZip = data.archivo.replace('.7z', '');
        const param_url = `https://tramite.heves.gob.pe/SGD/getFpParamsVBprincipal.php?nombreZip=${nombreZip}&iCodFirma=${iCodFirma}`;
        const paramPrev = {
            param_url: param_url,
            param_token: "123456",
            document_extension: "pdf"

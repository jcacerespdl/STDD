...
.panel-fijo{
  position: fixed; top: var(--stick-top); left:0; right:0;
  z-index: 950; background:#fff; box-shadow: 0 2px 0 rgba(0,0,0,.04);
}

.lista-scroll{
  margin-top: calc(var(--panel-h) + var(--panel-gap));
  padding-bottom: 24px;
  overflow-x: hidden; /* evita empuje horizontal */
}
...
.barra-titulo{
  ...
  width:100%; box-sizing:border-box; margin:0; z-index:910;
}

/* ===== Filtros ===== */
.filtros-formulario{
  display:flex; gap:30px; background:#fff;
  border:1px solid #ccc; border-radius:0;
  padding:20px 20px 10px;
  width:100%; box-sizing:border-box; flex-wrap:wrap; margin:0;
}
.columna-izquierda{
  flex:1; max-width:40%;
...
.input-container select{
  width:100%; padding:14px 12px 6px; font-size:14px;
  border:1px solid #ccc; border-radius:4px; background:#fff;
  box-sizing:border-box; appearance:none; height:42px; line-height:1.2;
}
.input-container select:required:invalid{ color:#aaa; }

.input-container label{
  position:absolute; top:50%; left:12px; transform:translateY(-50%);
  font-size:13px; color:#666; background:#fff; padding:0 4px;
...
.input-container select:valid + label{
  top:-7px; font-size:11px; color:#333; transform:translateY(0);
}
...
.btn-filtro{
  padding:0 16px; font-size:14px; border-radius:4px; min-width:120px;
  display:flex; align-items:center; justify-content:center; gap:6px;
  border:none; cursor:pointer; height:42px; box-sizing:border-box;
}
.btn-primary{ background:var(--primary); color:#fff; }
.btn-secondary{ background:var(--secondary); color:#fff; }

/* Grupo horizontal de checks */
.input-group{
  display:flex; align-items:center; gap:14px; height:42px;
}
.input-group .group-label{ font-size:13px; color:#666; white-space:nowrap; }
...
/* ===== Header sticky (encabezado de columnas) ===== */
.tabla-head-sticky{
  width:100%; background:#f6f7f9;
  border-top:1px solid #e7ebef; border-bottom:1px solid #e7ebef;
  font-weight:600; font-size:13.5px; color:#2f3a44;
...
.tabla-head-sticky .ths{
  ...
  grid-template-columns:
    ...
    var(--col-ext)
    var(--col-doc)
    minmax(var(--col-asunto-min), 2fr)
    var(--col-der)
    var(--col-est)
    var(--col-opt);
  align-items:center; justify-items:stretch; gap:0;
}
.tabla-head-sticky .th{
  padding:8px 12px; border-right:1px solid #edf1f4;
...
.grid-row{
  ...
  grid-template-columns:
    ...
    var(--col-opt);
  align-items:center; justify-items:stretch; gap:0;
  height:var(--row-h); overflow:hidden;
...
/* Ignorar anchos inline en celdas */
.grid-row > *{
  width:auto !important;
  justify-self:stretch !important;
}

/* Celdas centradas y con recorte */
.grid-row .cell,
.grid-row .grid-cell,
.grid-row .doc-col{
...
/* Expediente con sublíneas centradas */
.grid-row .cell.expediente-cell{
  flex-direction:column; align-items:center; text-align:center;
}
.grid-row .cell.expediente-cell small{ line-height:1.25; }

/* Documento con subfilas, sin romper alto fijo */
.grid-row .doc-col{ flex-direction:column; gap:2px; }
...
.badge-registros{
  ...
  font-size:12px; border-radius:999px; color:#fff; background:#1f2937;
...
/* Overrides cuando la paginación está en barra azul */
...
.btn.btn-link{
  ...
  color:var(--primary); font-size:18px; vertical-align:middle;
}
...
.modal-content{
  background:#fff; margin:5% auto; padding:20px; width:90%;
  max-width:1000px; border-radius:8px; position:relative;
}
...
.typeable-overlay{
  position:absolute; left:0; top:0; right:0; height:42px; display:none;
  border:1px solid #ccc; border-radius:4px; background:#fff;
...
.typeable-dropdown{
  ...
  max-height:240px; overflow:auto; display:none; z-index:2000; margin-top:4px;
}
...
.typeable-item.current{
  background:#e9f5ff; color:#005a86; font-weight:600; cursor:default;
  display:flex; align-items:center; justify-content:space-between;
}
...
<div class="contenedor-principal">
  ...
  <form class="filtros-formulario">
    ...
    <div class="columna-izquierda"> <!-- INICIO COLUMNA IZQUIERDA -->
      <div class="fila">

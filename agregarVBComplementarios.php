<?php
    ...
    if ($iCodTramite <= 0) {
        echo "<div style='color:red;'>Error: Código de trámite no válido {$iCodTramite}</div>";
        exit;
    }
?>
<form id="listadoFirmantes" style="margin-bottom: 1rem;">
    ...
    <div id="listadoFirmantesBody" style="display: flex; flex-direction: column; gap: 1.25rem; margint-top: 1rem;">
        <?php
            $sqlList = "SELECT T.cNombresTrabajador+' '+T.cApellidosTrabajador AS Trabajador, STRING_AGG(TD.cNombreOriginal, ', ') WITHIN GROUP (ORDER BY TD.iCodDIgital) AS Complementarios FROM Tra_M_Tramite_Firma TF JOIN Tra_M_Trabajadores T ON TF.iCodTrabajador = T.iCodTrabajador JOIN Tra_M_Tramite_Digitales TD ON TF.iCodDigital = TD.iCodDigital WHERE TF.iCodTramite = 278 GROUP BY T.cNombresTrabajador+' '+T.cApellidosTrabajador";
            $paramsList = array($iCodTramite);
            $queryList = sqlsrv_query($cnx,$sqlList,$paramsList);
            if($queryList == false){
            ...
            while($row = sqlsrv_fetch_array($queryList, SQLSRV_FETCH_ASSOC)){
                ?>
                    <div style="width: 100%, position: relative; border-radius: 8px; background-color: var(--light); padding: 1rem; display: flex; align-items: flex-start; flex-direction: column;">
                        <h4 style="font-weight: bold"><?= $row["Trabajador"]?></h4>
                        <?php
                            $arr = explode(", ", $row["Complementarios"]);
                            foreach($arr as $comple){
                                ?>
                                    <p style="font-size: var(--font-sm); opacity: 0.75"><?= $comple ?></p>
                                <?php
                            }
                        ?>
                    </div>
...
<form id="selectForm" style="margin-top:1rem; display: flex; flex-direction: column; gap: 1.25rem;">
    ...
    <div style="width: 100%; display: flex; flex-direction: column; gap: 1rem; margin-bottom: 1rem;">
        ...
        <button type="button" id="oficinaOpen" style="width: 100%; border-radius: 0.5rem; background-color: lightgray; padding: 0.5rem; cursor: pointer; display: flex; align-items: center; gap: 1rem; border: none; transition: background-color 0.2s ease-in-out;" >
        ...
        <div id="searchResults" style="display: flex; flex-direction: column; gap: 0.5rem; border-radius: 0.5rem; overflow: auto; max-height: 300px; width: 100%; padding: 0.75rem;">
            <?php
                ...
                while ($oficina = sqlsrv_fetch_array($rsOfic, SQLSRV_FETCH_ASSOC)) {
                    $oficinaObj[] = json_encode(["name" => $oficina["cNomOficina"], "code" => $oficina["cSiglaOficina"]]);
                    ?>
                        <button 
                            class="oficinaOption" 
